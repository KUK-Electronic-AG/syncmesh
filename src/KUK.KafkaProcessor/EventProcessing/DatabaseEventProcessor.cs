...
{
    public class DatabaseEventProcessor
    {
        private readonly IConfiguration _configuration;
        private readonly IEventCommandFactory _commandFactory;
        private readonly ILogger<DatabaseEventProcessor> _logger;
        ...
        public DatabaseEventProcessor(
            ...
            ILogger<DatabaseEventProcessor> logger,
            ...
            )
        {
            ...
            double memoryCacheExpirationInSeconds = Convert.ToDouble(_configuration["InternalKafkaProcessorParameters:MemoryCacheExpirationInSeconds"]);
            if (memoryCacheExpirationInSeconds < 1)
            {
                throw new ArgumentException($"Invalid memoryCacheExpirationInSeconds. Should be at least 1 second.");
            }
            ...
            _priorityLists = new List<List<PriorityDependency>>
            {
                ...
                // Address -> Customer: Customer zależy od Address, polem ID jest "AddressId"
                new List<PriorityDependency> {
        ...
        public async Task RunEventProcessingAsync()
        {
            ...
            await _kafkaService.CreateTopicAsync(_configuration["Kafka:OldToNewTopic"], 10, 3);
            await _kafkaService.CreateTopicAsync(_configuration["Kafka:SchemaChangesOldTopic"], 1, 3);
            await _kafkaService.CreateTopicAsync(_configuration["Kafka:DbHistoryOldTopic"], 1, 3);

            await RegisterOldConnector();
            double sleepForConnectorsInMs = Convert.ToDouble(_configuration["InternalKafkaProcessorParameters:SleepForBothConnectorsToBeUpdatedInMilliseconds"]);
            Thread.Sleep(Convert.ToInt32(sleepForConnectorsInMs)); // wait for status of old connector to be updated
            await WaitForBothConnectorsToBeOperational(true, false);

            await _connectorsRegistrationService.StopConnectorIfRunning("new-to-old-connector");
            ...
            {
                _logger.LogError($"Failure initializing new database. Going to return.");
                return;
            }
            ...
            {
                _logger.LogError($"Failure creating triggers in new database. Going to return.");
                return;
            }

            await RegisterNewConnector();

            // REMARK: There is issue creating these topics by new-to-old connector so we create them manually
            await _kafkaService.CreateTopicAsync(_configuration["Kafka:NewToOldTopic"], 10, 3);
            await _kafkaService.CreateTopicAsync(_configuration["Kafka:SchemaChangesNewTopic"], 1, 3);
            await _kafkaService.CreateTopicAsync(_configuration["Kafka:DbHistoryNewTopic"], 1, 3);

            await ValidateDebeziumTopics();
            Thread.Sleep(Convert.ToInt32(sleepForConnectorsInMs)); // wait for status of both connectors to be updated
            await WaitForBothConnectorsToBeOperational(true, true);
        ...
        private async Task WaitForBothConnectorsToBeOperational(bool waitForOldConnector, bool waitForNewConnector)
        {
            var mode = _utilitiesService.GetOnlineOrDockerMode();
            ...
            while (true)
            {
                ...
                if (waitForOldConnector)
                {
                    switch (mode)
                    {
                        case ApplicationDestinationMode.Docker:
                            oldConnectorStatus = await GetLocalDockerConnectorStatusAsync("old-to-new-connector");
                            break;
                        case ApplicationDestinationMode.Online:
                            oldConnectorStatus = await _externalConnectionService.GetConnectorStatusAsync("old-to-new-connector");
                            break;
                        default:
                            throw new InvalidOperationException($"Unknown mode {mode}");
                    }
                    oldOperational = !oldConnectorStatus.Contains("FAIL", StringComparison.OrdinalIgnoreCase);
                }
                ...
                if (waitForNewConnector)
                {
                    ...
                    {
                        case ApplicationDestinationMode.Docker:
                            newConnectorStatus = await GetLocalDockerConnectorStatusAsync("new-to-old-connector");
                            break;
                        case ApplicationDestinationMode.Online:
                            newConnectorStatus = await _externalConnectionService.GetConnectorStatusAsync("new-to-old-connector");
                            break;
                        default:
                            throw new InvalidOperationException($"Unknown mode {mode}");
                    }
                    newOperational = !newConnectorStatus.Contains("FAIL", StringComparison.OrdinalIgnoreCase);
                }
        ...
        private async Task RegisterNewConnector()
        {
        ...
        private async Task RegisterOldConnector()
        {
        ...
        private void ValidateAppSettingsConfiguration()
        {
            ...
            ValidateRequired("InternalKafkaProcessorParameters:SleepForBothConnectorsToBeUpdatedInMilliseconds");
        ...
        private async Task ValidateDebeziumTopics()
        {
            ...
            // An error occurred while fetching replication factor for topic old_to_new.db_chinook1: Sequence contains no elements
