...
using KUK.ChinookCrudsWebApp.Models;
using KUK.ChinookCrudsWebApp.Services.Interfaces;
using KUK.ChinookCrudsWebApp.Utilities;
...
namespace KUK.ChinookCrudsWebApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        private readonly IQuickContextActionsService _quickContextActionsService;
        ...
        public HomeController(
            ...
            IQuickContextActionsService quickContextActionsService,
            ...
            IExternalConnectionService externalConnectionService)
        {
            ...
            _quickContextActionsService = quickContextActionsService;
        ...
        public async Task<IActionResult> CreateOldCustomer()
        {
            try
            {
                ServiceActionStatus status = await _quickContextActionsService.QuickCreateOldCustomer();
                if (status.Success)
                {
                    return Ok(new { message = status.Message, success = true });
                }
        ...
        public async Task<IActionResult> CreateOldInvoice()
        {
            try
            {
                ServiceActionStatus status = await _quickContextActionsService.QuickCreateOldInvoice();
                if (status.Success)
                {
                    return Ok(new { message = status.Message, success = true });
                }
        ...
        public async Task<IActionResult> CreateNewCustomer()
        {
            try
            {
                ServiceActionStatus status = await _quickContextActionsService.QuickCreateNewCustomer();
                if (status.Success)
                {
                    return Ok(new { message = status.Message, success = true });
                }
        ...
        public async Task<IActionResult> CreateNewInvoice()
        {
            try
            {
                ServiceActionStatus status = await _quickContextActionsService.QuickCreateNewInvoice();
                if (status.Success)
                {
                    return Ok(new { message = status.Message, success = true });
                }
        ...
        public async Task<IActionResult> CreateNewAddress()
        {
            try
            {
                ServiceActionStatus status = await _quickContextActionsService.QuickCreateNewAddress();
                if (status.Success)
                {
                    return Ok(new { message = status.Message, success = true });
                }
        ...
        {
            var statuses = await _dockerService.GetContainerStatusesAsync(new[] { "kafka", "schema-registry", "control-center", "mysql80", "destinationpostgres", "martenpostgres", "debezium" });
            ...
            try
            {
                ...
                switch (mode)
                {
                    ...
                    case ApplicationDestinationMode.Online:
                        status = await _schemaInitializerService.InitializeSchemaTablesAndData(dbEnum, string.Empty);
                        break;
                    default:
        ...
        public async Task<IActionResult> StartProcessor()
        {
            try
            {
                ...
                // Wait for initialization and registration of new connector
                using (var scope = HttpContext.RequestServices.CreateScope())
                {
                    var processingService = scope.ServiceProvider.GetRequiredService<IProcessingService>();
                ...
                return Json(new { success = true, message = "Processor started successfully." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = $"Error launching processor: {ex.Message}" });
            }
        }
